...
500     if (L< lmax_extrap_highl) &
    ...
    do i=2,nthermo
      ...
      if (CP%Reion%Reionization .and. 1.0_dl/a-1.0_dl<Reionization_maxz) then
          ...
          ! THE FIRST POINT IS THE LAST ONE BEFORE THIS IF STATEMENT WAS TRUE
					! VM: SIDE NOTE: WE WANT recom_z TO BE AN INCREASING FUNCTION! 
        ...
        !VM WE NEED THAT WHEN CALCULATING REIONIZATION (BECAUSE OF SMOOTHING)
				...
				!VM i-ncount2+1 = 2 (firs call)
				!VM size = nthermo-ncount2+1
				!VM therefore size-1 = (nthermo-ncount2+1)-(i-ncount2+1)+1
    ...
    if (CP%Reion%Reionization .eq. .true.) then
    ...
    if (CP%Reion%Reionization) then
      !VM: BEGINS
      !VM: IMPLEMENTS THE CONCEPT OF ACCURACY BOOST FOR REIONIZATION
